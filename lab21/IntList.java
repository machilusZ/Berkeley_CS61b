...
public class IntList {
    ...
    private IntListNode insert(IntListNode p, IntListNode head) {
        if(head==null)return p;
            ...
            IntListNode pos=p;
            for(IntListNode cur=p; cur.prev!=null&&cur.prev.item>p.item;cur=cur.prev)
                pos=cur.prev;
            if(pos==p) return head;
            p.prev.next=p.next;
    ...
    public IntList quicksort() {
        ...
        IntListNode cur=head.next;
        while(cur != null) {
            if(cur.item<divider) {
                smallElements.addToEnd(cur.item);
            } else {
                largeElements.addToEnd(cur.item);
            }
            cur=cur.next;
        }
